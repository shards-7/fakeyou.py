...
"""
Objects to return data in a dynamic way, no i won't explain it all
    ...
    def __init__(self, json: dict, size):
        ...
        if size != 0:
            for i in range(size):
                ...
                self.creatorDisplayName.append(
                    voicels[i]["creator_display_name"])
                self.creatorGavatarHash.append(
                    voicels[i]["creator_gravatar_hash"])
                self.title.append(voicels[i]["title"])
                self.langTag.append(voicels[i]["ietf_language_tag"])
                self.isFrontPageFeatured.append(
                    voicels[i]["is_front_page_featured"])
    ...
    def __init__(self, json, size):
        ...
        if size != 0:
            for i in range(size):
                ...
                self.maybeSuperCategoryToken.append(
                    categoriesls[i]["maybe_super_category_token"])
                self.canDirectlyHaveModels.append(
                    categoriesls[i]["can_directly_have_models"])
                self.canHaveSubCategories.append(
                    categoriesls[i]["can_have_subcategories"])
                self.onlyModsApply.append(
                    categoriesls[i]["can_only_mods_apply"])
...
class wav():
    ...
    def __init__(self, hjson, content=None):
        ...
        self.link = self.link = "https://storage.googleapis.com/vocodes-public" + \
            str(json["maybe_public_bucket_wav_audio_path"])
...
class events():
    ...
    def __init__(self, ejson):
        ...
        for json in ls:
            ...
            self.maybeGavatarHash.append(
                json["maybe_target_user_gravatar_hash"])
...
class _w2l_results():
    def __init__(self, w2lJson):
        ...
        for data in self.json:
            ...
            self.maybeTtsInferenceResultToken.append(
                data["maybe_tts_inference_result_token"])
...
class _tts_models():
    def __init__(self, modelsJson):
        ...
        for data in self.json:
            ...
            self.ietfPrimaryLanguagTag.append(
                data["ietf_primary_language_subtag"])
            ...
            self.suggestedUniqeBotCommand.append(
                data["maybe_suggested_unique_bot_command"])
            self.created_date.append(data["created_at"])
            self.update_date.append(data["updated_at"])
...
class _w2l_templates():
    def __init__(self, tempJson):
        ...
        for data in self.json:
            ...
            self.isPublicListingApproved.append(
                data["is_public_listing_approved"])
            self.created_date.append(data["created_at"])
...
class w2lo():
    ...
    def __init__(self, stateJson, content):
        ...
        self.link = "https://storage.googleapis.com/vocodes-public" + \
            str(ij["maybe_public_bucket_video_path"])
