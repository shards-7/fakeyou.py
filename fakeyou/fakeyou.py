...
class FakeYou():
    ...
    def make_tts_job(self, text: str, ttsModelToken: str):
        ...
        payload = {"uuid_idempotency_token": str(
            uuid4()), "tts_model_token": ttsModelToken, "inference_text": text}
        handler = self.session.post(
            url=self.baseurl+"tts/inference", data=json.dumps(payload))
        if handler.status_code == 200:
            ijt = handler.json()["inference_job_token"]
            return ijt
        elif handler.status_code == 400:
            raise RequestError(
                "check token and text, or contact the developer IG:@thedemonicat")
    ...
    def tts_status(self, ijt: str):
        ...
        if handler.status_code == 200:
            ...
            raise RequestError(
                "Something went wrong, content:", handler.content)
    ...
    def get_user(self, username: str, limit: int = 25):
        try:
            profile_handler = self.session.get(
                self.baseurl+f"user/{username}/profile")
            if profile_handler.status_code == 404:
                raise UserNotFound(username)
            tts_results = self.session.get(
                self.baseurl+f"user/{username}/tts_results?limit={limit}")
            w2l_results = self.session.get(
                self.baseurl+f"user/{username}/w2l_results?limit={limit}")
            tts_models = self.session.get(
                self.baseurl+f"user/{username}/tts_models?limit={limit}")
            w2l_models = self.session.get(
                self.baseurl+f"user/{username}/w2l_templates?limit={limit}")
    ...
    def get_queue(self):
        try:
            queue = self.session.get(
                "https://api.fakeyou.com/tts/queue_length").json()["pending_job_count"]
            return queue
        except:
            raise RequestError(
    ...
    def create_account(self, username: str, password: str, email: str):
        ...
        data = {"username": username, "email_address": email,
    ...
    def w2l_poll(self, ijt: str):
        while True:
            polling_handler = self.session.get(
                f"https://api.fakeyou.com/w2l/job/{ijt}")
            if polling_handler.status_code == 200:
                try:
                    pjs = polling_handler.json()
                except:
                    raise RequestError(
                        "Something went wrong.", "REQUEST CONTENT:", polling_handler.content)
                state = pjs["state"]["status"]
                if state == "complete_success":
                    content = self.session.get("https://storage.googleapis.com/vocodes-public"+str(
                        pjs["state"]["maybe_public_bucket_video_path"])).content
    ...
    def delete_tts_result(self, result_token):
        ...
        if stats == 200:
            ...
            raise UnAuthorized(
                f"You're not authorized to delete tts result {result_token}")
        elif stats == 404:
